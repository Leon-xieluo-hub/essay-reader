...
export function UploadDropzone({ compact = false }: { compact?: boolean }) {
  ...
  const ocrToggle = (
    <label
      ...
      title="扫描件（没有文本层）需要 OCR；文本层 PDF 会自动跳过 OCR"
  ...
  return (
    <div
      ...
      onDrop={(event) => {
        ...
        else useApp.getState().toast("warn", "只支持 PDF 文件");
      }}
    >
      <input
        ref={inputRef}
      ...
      {uploading ? (
        <div className="text-xs text-ink-2">
          ...
          <div className="mt-1 text-[11px] text-ink-3">
            {uploading.percent < 100 ? `上传中 ${uploading.percent}%` : "解析中（扫描件 OCR 约 1~3 秒/页）…"}
          </div>
        </div>
      ) : compact ? (
        ...
        <div className="space-y-3">
          <button className="w-full text-center" onClick={pick}>
            ...
            <div className="mt-3 text-sm font-medium">拖入 PDF，或点击选择文件</div>
