...
- **Borderless tables are reconstructed**: many journal tables only have a top and a bottom rule and no vertical lines, so PyMuPDF's table detection finds nothing at all (0 tables for the whole test paper). The fallback merges text fragments on the same baseline into rows, splits cells on column gaps ≥12 pt, and requires column positions to align across most rows; "formula / reference list" signatures veto false positives (otherwise equations and reference lists turn into tables). Rows whose left cell wraps onto a second line — the right-hand value then sits *between* the two halves and baseline grouping never sees a pair, which used to drop the row out of the table entirely — are absorbed explicitly. On the test paper the 17-row two-column table comes out as 18 rows (header + 17 traits, `FSI` included), and translation keeps the row/column correspondence (header → `性状/缩写`, abbreviations such as `PH`/`PW` untouched).
- **Tables are stripped first**: `page.find_tables()` marks table regions, and text lines inside them are removed from the body flow so cells are not read as prose and cannot invent a phantom "second column". The table itself is rebuilt as structured rows/columns plus HTML, scrollable in the UI and comparable against the original page. The same step rejects "fake tables": if the text in a candidate region runs full width (a reference list, say), it is not treated as one. **A table may only swallow text it reproduces**: the characters of the removed lines are compared with the rebuilt cells, and when coverage is below 80% the original text is kept in the flow (with a warning) instead of disappearing.
- **Display equations are cropped as images**: the text layer returns equations **scrambled** — sub/superscripts, braces and summation limits each become separate lines (`\x1a \x1b b∈B ∥a −b ∥ h(A, B) = max min`), and no text post-processing can restore them reliably. Equations are therefore located by region, anchored on the **equation number `(n)`**: fragments near that baseline are the equation body (including bare glyph runs such as `z`, `ph`, `V =`), each equation becomes its own image, and the crop spans the full horizontal extent of the line so the left edge is not cut off. **Prose is never part of a crop**: a sentence that merely shares the band is excluded from the crop rectangle and stays in the body flow, and a paragraph that only mentions an equation (`…with R² = 0.72…`) is not retyped as a formula at all — it keeps its typeface and is still translated. On the test paper all 4 equation images (including the MAPE summation) come out complete.
...
- **Paragraphs cut by the layout are rejoined**: a paragraph broken by a page break, or interrupted by a floated figure, is put back together. The signal is typographic rather than stylistic: a last line that **reaches the column's right edge** was cut by the measure and therefore continues — this works for the block-style layouts MDPI, Frontiers and Elsevier use, where no paragraph is indented (indent-based detection fires on only 1–13% of paragraphs across our nine-paper test set). A merge is refused when the next block opens a section heading, a run-in label such as `Stem length:`, a caption, or when a table/equation sits between the halves; across a page break a float may be crossed only when the text also resumes mid-sentence in lowercase.
- **Inline styling is kept**: bold and italic are extracted per span (`emphasis` ranges on every block, offsets into its text) and rendered as `<strong>` / `<em>` in the reading pane, composed with search highlighting.
...
- **The UI follows your system language**: Chinese browsers get Simplified Chinese, everything else gets English, and the `中 / EN` button in the top bar switches and remembers the choice (313 strings, no untranslated keys).
- The **outline sits directly under the open paper** in the sidebar rather than after the whole library, so a long library list no longer pushes it out of reach.
- Switching **Bilingual / Translation / Original** keeps your place: the paragraph at the top of the viewport is pinned there, because every block changes height when the mode changes and the browser would otherwise clamp the scroll position.
- Theme follows the system (light cream / dark) and can be pinned; font scale is adjustable and remembered.
